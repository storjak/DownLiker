...
function initialToggleEmbed() {
    let feedDetector = setInterval(() => {
        clickListenerHook = document.getElementsByTagName("main");
...
function likeClickHandler(e) {
    ...
    if (likesElement) {
        const article = e.composedPath().find(x => {
            return x.tagName === "ARTICLE";																		 			//  <<<   maybe I can simplify this too
        }),
